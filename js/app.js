...
// 판단을 프롬프트 규칙으로 옮긴 것이라 임의 추측이 아님. 출력 형식은 기존과 동일한 2열(한국어\t영어)
// 유지 — 무료판 사용자가 결과를 다시 붙여넣을 때 쓰는 parseSentencesText()가 2열만 이해하기 때문.
function buildAiVariationPrompt(sentence) {
  return `다음 한국어-영어 문장을 아래 9가지 방식으로 변형해줘. 각 방식이 원문의 의미상 자연스럽게 성립하는 경우에만 만들고, 아래에 해당하면 그 방식은 억지로 만들지 마:
...
각 줄에 "한국어 문장" 다음 탭(Tab)을 하나 넣고 이어서 "English sentence"를 쓰는 형식으로, 결과를 코드블록 안에 한 줄에 하나씩만 적어줘(설명 없이 결과만).
