...
// 문장관리 노트형 학습(25번) — 전체보기/한글만/영어만 표시 방식. 필터(전체/중요/미암기)와
// 달리 재방문해도 유지되는 개인 취향이라 localStorage에 저장
const LIST_DISPLAY_MODE_KEY = 'tuktak_list_display_mode';
let listDisplayMode = localStorage.getItem(LIST_DISPLAY_MODE_KEY) || 'both';
...
variationDetailBodyEl.addEventListener('click', (e) => {
  ...
  if (addBtn) {
    ...
    if (variationSelectedIndexes.has(flatIndex)) {
      ...
      addBtn.innerHTML = VARIATION_ADD_ICON;
    } else {
      variationSelectedIndexes.add(flatIndex);
      addBtn.classList.add('selected');
      addBtn.innerHTML = VARIATION_ADD_ICON_SELECTED;
    }
    updateVariationDetailFooter();
    return;
  }
...
// 아래 8가지 예외 규칙은 15-1(문장변형 체험판) 87개 예문을 손수 만들 때 실제로 적용했던 판단 기준을
// 일반화한 것(예: 9번 날씨 문장은 인칭·수 생략, 시간부사 있는 문장은 충돌 시제 생략 등) — 검증된
// 판단을 프롬프트 규칙으로 옮긴 것이라 임의 추측이 아님. 다만 이 8개는 "예시"일 뿐 전체 목록이
// 아니라서, 여기 없는 새 케이스는 계속 놓칠 수 있음 — 매번 규칙을 추가하는 방식은 한계가 있다는
// 사용자 지적(2026-09-19)으로 "원어민이 실제로 이렇게 말할지 스스로 판단하라"는 일반 원칙을 목록
// 앞에 추가했으나, 실제 Anthropic API로 검증한 결과 이 일반 원칙만으로는 "현재완료시제 변형을
// 만들어라"는 명시적 요청 자체의 압력을 못 이겨 "커피 마시고 싶다" → "I have wanted to have a
// coffee." 같은 부자연스러운 결과가 그대로 나옴을 확인(`thinking` 모드를 켜고 재검증해도 동일 —
// "생각할 여유 부족"이 원인이 아니라 구조적 문제). 그래서 이 구체 사례를 8번째 규칙으로 추가해
// 실제 API로 재검증(해결 확인 + "오랫동안 좋아해왔다"처럼 지속기간 부사가 있어 현재완료가 맞는
// 문장은 과잉 차단되지 않는 것도 확인) — 일반 원칙은 아직 발견 못한 케이스를 위한 안전망으로
// 유지하되, 실제로 검증된 해법은 계속 구체적 규칙을 늘리는 쪽. 출력 형식은 기존과 동일한 2열
// (한국어\t영어) 유지 — 무료판 사용자가 결과를 다시 붙여넣을 때 쓰는 parseSentencesText()가
// 2열만 이해하기 때문.
function buildAiVariationPrompt(sentence) {
  return `다음 한국어-영어 문장을 아래 9가지 방식으로 변형해줘.

변형을 만들기 전에 매번 "원어민이 실제 대화에서 이렇게 말할까?"를 스스로 판단해라. 문법적으로는 성립해도 실제로는 어색하거나 잘 안 쓰는 표현이면 그 변형은 만들지 마라 — 누군가 어떤 문장을 보여주며 "이거 자연스러운 표현이야?"라고 물었을 때처럼 판단하면 된다. 아래는 그 판단 기준의 대표적인 예시일 뿐이니, 목록에 없는 경우에도 같은 기준으로 스스로 걸러라:

- 인사/감사/사과 등 고정 관용표현 → 시제·의문문 변형 생략 (예: "안녕하세요" / "Hello.")
- 비인칭 주어 문장(날씨, "~이 있다" 등) → 인칭(2인칭/3인칭 단수) 변형 생략 (예: "비가 온다" / "It's raining.", "책상 위에 책이 있다" / "There is a book on the desk.")
- 청유문/명령문("~하자") → 시제 변형 생략 (예: "같이 가자" / "Let's go.")
- 문장에 이미 시간부사(어제/내일/지금 등)가 있는 경우 → 그 시간부사와 충돌하는 시제 변형 생략 (예: "어제 학교에 갔다" / "I went to school yesterday." → 미래시제 생략)
- 이미 부정 의미를 내포한 관용구("전혀 모르겠다" 등) → 부정문 변형 생략(이중부정 방지)
- "유일한", 고유명사 주어 등 복수화가 논리적으로 성립 안 되는 경우 → 복수 변형 생략 (예: "나는 외동이다" / "I am the only child.")
- 감탄문("~구나!") → 의문문 변형 생략(뉘앙스가 사라지고 의미가 바뀜)
- 즉흥적 욕구·감정·신체상태 표현("~하고 싶다", "피곤하다", "졸리다" 등)은 "오랫동안/쭉" 같은 지속 기간 표현이 없으면 현재완료로 쓰면 부자연스러움 → 현재완료 변형 생략 (예: "커피 마시고 싶다" / "I want to have a coffee.")

판단이 끝나면, 자연스럽다고 확인된 변형만 아래 형식으로 출력해라: 각 줄에 "한국어 문장" 다음 탭(Tab)을 하나 넣고 이어서 "English sentence"를 쓰는 형식으로, 결과를 코드블록 안에 한 줄에 하나씩만 적어줘(설명 없이 결과만).
...
cloudBackupInfoBtn.addEventListener('click', () => {
  alert('클라우드에 이미 저장된 문장과 지금 기기의 문장을 합쳐서 클라우드(인터넷의 저장공간)에 저장합니다.');
});

cloudRestoreInfoBtn.addEventListener('click', () => {
  alert('클라우드(인터넷의 저장공간)에 저장해둔 문장을 가져옵니다.\n이 기기의 문장은 사라지므로 핸드폰에서 추가한 문장이 있으면 "클라우드에 백업" 버튼을 누르세요.');
});

cloudDeleteInfoBtn.addEventListener('click', () => {
  alert('클라우드에 저장된 문장들을 삭제합니다.\n이 기기에서 추가한 문장들을 삭제하려면 문장관리 메뉴를 사용하세요.');
});
  ...
  try {
    ...
    (cloudSentences || []).map(normalizeSentence).forEach((cs) => {
      ...
      if (match) {
        ...
        match.important = match.important || cs.important;
        match.unfamiliar = match.unfamiliar || cs.unfamiliar;
        if (`${match.important}-${match.unfamiliar}` !== before) updatedFlagCount++;
      } else {
        const newSentence = makeSentence(cs.kr, cs.en, cs.source);
        newSentence.important = !!cs.important;
        newSentence.unfamiliar = !!cs.unfamiliar;
        sentences.push(newSentence);
        randomOrder.push(String(newSentence.id));
        addedCount++;
      }
    });
...
cloudRestoreBtn.addEventListener('click', async () => {
  if (!confirm('클라우드에 백업된 문장으로 지금 기기의 문장을 모두 덮어씁니다. 이 기기에서 최근에 추가하거나 수정한 문장이 있다면 먼저 "클라우드에 백업"으로 반영해두는 것을 추천합니다. 계속할까요?')) return;
